...
I was looking for a way to plot syri-output, similar to what [`plotsr`](https://github.com/schneebergerlab/plotsr/) does, but with easier customization and in R. I could not find anything, so I wrote something. The files included here in `data/` for demonstration are the plotsr example files.

This requires 'tidyverse' (`dplyr`, `magrittr`, and `vroom`) and [`gggenomes`](https://github.com/thackl/gggenomes). This repo also comes with a snapshot that can be used with `renv::restore()`
